...
### 데이터 현행성 검증 (2026-06-18)
- 100건 전수를 공식 출처(복지로·소관부처 `*.go.kr`·각 공단)로 **2026년 기준** 검증·교정. 결과·출처·신뢰도는 **`VERIFICATION_LOG.md`** 에 정리.
- 결과: amount 100건·eligibility 97건·name 39건 갱신(폐지·개편 정책 반영 — 예: 장애인 자동차 'LPG 보조금'은 폐지되어 세제감면으로 정정, 청년도약계좌→2026 청년미래적금 개편, 에너지바우처 2026 하·동절기 통합 금액).
- confidence: **high 61 · medium 39 · low 0**. 확인 불가한 구체 수치는 틀린 숫자 대신 정성 표현으로 처리.
- ⚠️ **medium(39건)·uncertain(1건)은 보도·2차 출처 기반 또는 지자체 차등** → 실제 대회 제출 전 `VERIFICATION_LOG.md`의 출처로 최종 spot-check 권장. `phone` 번호도 제출 전 실통화 확인 권장.
